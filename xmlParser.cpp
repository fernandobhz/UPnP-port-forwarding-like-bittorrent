...
int XMLNode::ParseClearTag(void *px, void *pa)
{
	...
	//xyz: http://stackoverflow.com/questions/14682480/converting-from-const-char-to-lptstr-without-uses-converstion
	//lpszTemp = _tcsstr(lpszXML, pClear->lpszClose);
    lpszTemp = const_cast<LPSTR>(_tcsstr(lpszXML, pClear->lpszClose));
...
XMLNode XMLNode::getChildNode(LPCTSTR name, int *j)
{
    ...
    for (; i<n; i++)
    {
		...
		p = const_cast<LPSTR>(strstr(pc->d->lpszName, ":"));
